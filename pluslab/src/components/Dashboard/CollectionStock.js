...
const CollectionStock = () => {
  ...
  }, [resultsValues, myStockCollectionTemp]);
  ...
  const addOrRemoveCollection = (e, n) => {
    ...
    for (let m = 0; m < temp.length; m++) {
      if (n == myStockCollection[m].key && temp[m].collection == true) {
        myStockCollection[m].collection = false;
        temp[m].collection = false;
        setMyStockCollectionTemp(myStockCollection);
        setOriginData(temp);

        return;
      }
      if (n == myStockCollection[m].key && temp[m].collection == false) {
        myStockCollection[m].collection = true;
        temp[m].collection = true;
        setMyStockCollectionTemp(myStockCollection);
        setOriginData(temp);
            ...
            >
              <td>
                <svg
                  ...
                  onClick={(e) => addOrRemoveCollection(e, originData.key)}
                >
                  <path
